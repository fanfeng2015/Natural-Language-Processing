...
Word embeddings is a technique that maps words to vectors of real numbers. It can be generated by methods such as neural networks, dimensionality reduction on the word co-occurrence matrix, etc. Word embeddings can be useful in a variety of NLP tasks, such as sentiment analytis, syntactic parsing, named-entiry recognition, etc.

Pretrained word embeddings can be useful in NLP tasks because it is able to capture context of a word in a document, such as its semantic/syntactic similarity with other words, relationships to other words, etc, while the traditional bog-of-words model doesn't capture these information.
...
RNNs is a class of artificial neural networks that perform the same task for every element of the input sequence. RNNs use internal state (memory) to process sequences of inputs, so as to capture information about what has been calculated so far. In other words, output of each node depends on computations on previous nodes.

One problem of RNNs is vanishing gradients, LSTM solves the vanishing gradients problem by using a cell with 3 gates (input gate, output gate, and forget gate).

Bidirectional LSTM, in addition, connects two hidden layers of opposite directions to the same output, instead of one hiddent layer as in unidirectional LSTM. The benifit is that the output layer can get information from both past states and future states, thus capturing more information about the input sequence.
